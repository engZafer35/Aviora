...
//Maximum number of file descriptors a c_fd_set object can hold
#ifndef C_FD_SETSIZE
   #define C_FD_SETSIZE 8
#elif (C_FD_SETSIZE < 1)
   #error C_FD_SETSIZE parameter is not valid
#endif
...
//Address families
#define C_AF_UNSPEC          0
#define AF_INET              2
#define AF_INET6             10
#define AF_PACKET            17

//Protocol families
#define PF_UNSPEC            C_AF_UNSPEC
...
typedef struct c_fd_set
{
   int_t fd_count;
   int_t fd_array[C_FD_SETSIZE];
} c_fd_set, FD_SET, *PFD_SET;
...
//Forward declaration of functions
void socketFdZero(c_fd_set *fds);
void socketFdSet(c_fd_set *fds, int_t s);
void socketFdClr(c_fd_set *fds, int_t s);
int_t socketFdIsSet(c_fd_set *fds, int_t s);
...
struct hostent *c_gethostbyname_r(const char_t *name, struct hostent *result,
   char_t *buf, size_t buflen, int_t *h_errnop);

int_t c_getaddrinfo(const char_t *node, const char_t *service,
