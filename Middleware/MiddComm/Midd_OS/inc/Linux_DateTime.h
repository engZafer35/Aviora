...
const char *formatSystemTime(systime_t time, char *str);
const char *formatDate(const DateTime *date, char *str);
...
int compareDateTime(const DateTime *date1, const DateTime *date2);

uint8_t computeDayOfWeek(uint16_t y, uint8_t m, uint8_t d);

int linuxLocalTimeSvcSetEpoch(unsigned int epoch);
