   ...
   if(path != NULL)
   {
      ...
      if(dir != NULL)
      {
         ...
         if(dir->handle != NULL)
         {
            ...
            strncpy(dir->path, path, FS_MAX_PATH_LEN);
         }
         else
         {
 ...
 * @return Error code
 **/
static void pathCombine(char *path, const char *name, size_t maxLen)
{
    size_t len = strlen(path);

    // Eğer path boşsa direkt name kopyala
    if (len == 0) {
        snprintf(path, maxLen, "%s", name);
        return;
    }

    // Son karakter '/' değilse ekle
    if (path[len - 1] != '/') {
        snprintf(path + len, maxLen - len, "/%s", name);
    } else {
        snprintf(path + len, maxLen - len, "%s", name);
    }
}
error_t fsReadDir(FsDir *dir, FsDirEntry *dirEntry)
{
   ...
   if(entry != NULL)
   {
      ...
      strncpy(dirEntry->name, entry->d_name, FS_MAX_NAME_LEN);
      ...
      strncpy(path, dir->path, FS_MAX_PATH_LEN);
